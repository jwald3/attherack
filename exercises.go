...
// ExerciseLibrary holds the embedded dataset plus any user-added exercises and
...
func (l *ExerciseLibrary) Add(exs ...Exercise) {
	l.mu.Lock()
	defer l.mu.Unlock()
	for _, e := range exs {
...
func (l *ExerciseLibrary) Count() int {
	l.mu.RLock()
	defer l.mu.RUnlock()
	return len(l.all)
}
...
func (l *ExerciseLibrary) Facets() (muscles, equipment []Facet) {
	l.mu.RLock()
	defer l.mu.RUnlock()
...
func loadExercises(raw []byte) (*ExerciseLibrary, error) {
	var all []Exercise
	if err := json.Unmarshal(raw, &all); err != nil {
		return nil, err
	}
	lib := &ExerciseLibrary{all: all, byID: make(map[string]Exercise, len(all))}
	for _, e := range all {
		lib.byID[e.ID] = e
	}
	return lib, nil
...
func (l *ExerciseLibrary) Search(query, muscle, equipment string, limit int) []Exercise {
	if limit <= 0 {
		limit = 25
	}
...
func (l *ExerciseLibrary) Get(id string) (Exercise, bool) {
	l.mu.RLock()
	defer l.mu.RUnlock()
	e, ok := l.byID[id]
	return e, ok
...
func (l *ExerciseLibrary) byName(name string) (Exercise, bool) {
	l.mu.RLock()
	defer l.mu.RUnlock()
