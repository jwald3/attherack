...
// seedDemo fills an empty database with about six weeks of realistic fake
...
SELECT (SELECT COUNT(1) FROM workouts) + (SELECT COUNT(1) FROM cardio_sessions) +
       ...
       (SELECT COUNT(1) FROM food_logs) + (SELECT COUNT(1) FROM chat_threads)`).Scan(&n); err != nil {
		return err
	}
	if n > 0 {
	...
	day := func(i int) string { return start.AddDate(0, 0, i).Format("2006-01-02") }
